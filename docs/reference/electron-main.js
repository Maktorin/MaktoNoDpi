  ...
  try {
    return await new Promise((resolve, reject) => {
      const req = https.get('https://api.github.com/repos/bol-van/zapret/releases/latest', {
        ...
        headers: { 'User-Agent': 'MaktoNoDpi' },
        timeout: 30000
      }, (res) => {
        if (res.statusCode === 302 || res.statusCode === 301) {
          const rReq = https.get(res.headers.location, { family: 4, lookup: ipv4Lookup, headers: { 'User-Agent': 'MaktoNoDpi' }, timeout: 30000 }, (r) => {
            let data = '';
...
function getResourcePath() {
  ...
  // process.resourcesPath. Using userData (~/.../MaktoNoDpi/) is always writable.
...
async function downloadAndExtractBinaries() {
  ...
  const tempDir = path.join(app.getPath('temp'), 'maktonodpi-temp');
...
function disableQuicBlock() {
  ...
  try {
    ...
    try {
      sudo.exec('/sbin/pfctl -f /etc/pf.conf 2>/dev/null; exit 0', { name: 'MaktoNoDpi' }, () => {});
    } catch (e2) {}
  }
}
...
async function startProxyWindowsElevated(finalBinaryPath, strategies, totalStrategies) {
  ...
  const progressFile = path.join(tempDir, 'maktonodpigress.txt');
  ...
  const hostsUpdateScript = path.join(tempDir, 'maktonodpi-update-hosts.ps1');
  ...
  const result = await new Promise((resolve) => {
    sudo.exec(`"${batchFile}"`, { name: 'MaktoNoDpi' }, (error) => {
...
async function startProxy() {
  ...
  if (process.platform === 'win32') {
    ...
    if (isRunningAsAdmin()) {
      const psPath = path.join(tempDir, 'maktonodpi-update-hosts.ps1');
...
const HOSTS_MARKER = '# MaktoNoDpi Discord/Telegram hosts';
...
async function prepareHostsUpdateForBatch(tempDir) {
  const tempFile = path.join(tempDir, 'maktonodpi-hosts-discord.txt');
  const psScriptPath = path.join(tempDir, 'maktonodpi-update-hosts.ps1');
...
async function updateHostsMacOS() {
  ...
  const tempFile = path.join(app.getPath('temp'), 'maktonodpi-hosts-add.txt');
  const block = '\n\n' + HOSTS_MARKER + '\n' + hostsData;
    ...
    const req = https.get(HOSTS_URL, { family: 4, lookup: ipv4Lookup, timeout: 15000 }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          sudo.exec(`powershell -ExecutionPolicy Bypass -NoProfile -File "${psScriptPath.replace(/\\/g, '\\\\')}"`, { name: 'MaktoNoDpi update hosts' }, (err) => {
